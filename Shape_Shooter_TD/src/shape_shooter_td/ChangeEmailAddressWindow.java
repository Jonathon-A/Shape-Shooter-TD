...
final public class ChangeEmailAddressWindow extends Shape_Shooter_TDAssets implements WindowFormatInterface {

    @Override
    final public void StartSetup(final int TransitionType) {
        /*
        This method plays the transition between panels
        It also opens the change email address panel and closes the previous panel while the transition is opaque
         */

        //Plays start of transition        
        Shape_Shooter_TD.MainClass.TransitionStart(TransitionType);
        //Opens change email address panel
        ChangeEmailAddressWindowSetup();
        //Removes previous panel before playing end of transition
        Shape_Shooter_TD.MainClass.TransitionEnd();
        //Change email address panel is currently displayed
        Shape_Shooter_TD.MainClass.setCurrentDisplayedPanel(ChangeEmailAddressPanel);
    }

    private final JLayeredPane ChangeEmailAddressPanel = new JLayeredPane();

    private void ChangeEmailAddressWindowSetup() {
        /*
        This method opens the change email address panel
        ...
        //Adds title image
        TitleSetup(ChangeEmailAddressPanel);
        //Setup change email address area components 
        ChangeEmailAddressAreaSetup();
        ...
        //Setup change email address and cancel buttons 
        ChangeEmailAddressAndCancelButtonSetup();
        //Setup account registration primary panel
        FinalSetup();
    }

    private void ChangeEmailAddressAreaSetup() {
        /*
        This method sets the properties of the change email address area components
        It then adds them to the change email address panel
         */

        //Sets the properties of the change email address area
        final JPanel ChangeEmailAddressArea = new JPanel();
        ChangeEmailAddressArea.setBorder(AssetsBorderArray[0]);
        ChangeEmailAddressArea.setBounds(20, 644, 990, 300);
        ChangeEmailAddressArea.setBackground(AssetsColorArray[0]);
        //Adds the change email address area at the back most position
        ChangeEmailAddressPanel.add(ChangeEmailAddressArea, new Integer(1));
        //Sets the properties of the change email address title
        final JTextField ChangeEmailAddressTitle = new JTextField();
        ChangeEmailAddressTitle.setBounds(20, 644, 990, 60);
        ChangeEmailAddressTitle.setFont(AssetsFontArray[5]);
        ChangeEmailAddressTitle.setEditable(false);
        ChangeEmailAddressTitle.setText("Enter email address and confirm ownership");
        ChangeEmailAddressTitle.setHorizontalAlignment(JTextField.CENTER);
        ChangeEmailAddressTitle.setOpaque(false);
        ChangeEmailAddressTitle.setBorder(null);
        //Adds the change email address title at the front most position
        ChangeEmailAddressPanel.add(ChangeEmailAddressTitle, new Integer(2));
        //Sets the properties of the change email address description
        final JTextField ChangeEmailAddressDescription = new JTextField();
        ChangeEmailAddressDescription.setBounds(20, 688, 990, 36);
        ChangeEmailAddressDescription.setFont(AssetsFontArray[2]);
        ChangeEmailAddressDescription.setEditable(false);
        ChangeEmailAddressDescription.setText("Please enter a different unique email address and confirmation code that will be emailed to you");
        ChangeEmailAddressDescription.setHorizontalAlignment(JTextField.CENTER);
        ChangeEmailAddressDescription.setOpaque(false);
        ChangeEmailAddressDescription.setBorder(null);
        //Adds the change email address description at the front most position
        ChangeEmailAddressPanel.add(ChangeEmailAddressDescription, new Integer(2));
    }
    ...
    private void EmailAddressEntrySetup() {
        ...
        ChangeEmailAddressPanel.add(EmailAddressEntryField, new Integer(2));
        ...
        //Adds the email address validation indicator at the front most position.0
        ChangeEmailAddressPanel.add(EmailAddressValidationDisplay, new Integer(3));
    }
    ...
    private void EmailAddressEntryUpdate(final String EnteredEmailAddress) {
        ...
        if (matcher.matches() && UniqueEmailAddress && !EnteredEmailAddress.equals("") && ValidEmailAddressLength) {
            ...
            if (UniqueEmailAddress) {
                ...
                if (EnteredEmailAddress.equals(Shape_Shooter_TD.MainClass.Shape_Shooter_TD_Database.getCurrentUserAccountEntry().getEmailAddress())) {
                    //Indicator displays "Taken";
                    EmailAddressValidationDisplay.setText("Same");
                } else {
                    //Indicator displays "Taken";
        ...
        //Updates confirmation code send and change email address button
        ConfirmationCodeSendButton.setText("Send");
        //Incorrect confirmation code is entered
        ChangeEmailAddressButtonUpdate("");
    ...
    private void ConfirmationCodeEntrySetup() {
        ...
        ConfirmationCodeEntryField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void insertUpdate(final DocumentEvent de) {
                ...
                ChangeEmailAddressButtonUpdate(ConfirmationCodeEntryField.getText());
        ...
        ChangeEmailAddressPanel.add(ConfirmationCodeEntryField, new Integer(2));
        ...
        ChangeEmailAddressPanel.add(ConfirmationCodeSendButton, new Integer(3));
    }
    ...
    private void ChangeEmailAddressButtonUpdate(final String EnteredConfirmationCode) {
        /*
        This method checks whether or not the correct confirmation code has been entered
        Prevents the user from changing their account's email if an incorrect confirmation code has been entered
         */
        ...
        if (EnteredConfirmationCode.equals(String.valueOf(ConfirmationCode))) {
            ...
            ChangeEmailAddressButton.setBackground(AssetsColorArray[0]);
            ValidDetailsEntered = true;
        }//If the correct confirmation code has not been entered
        else {
            ChangeEmailAddressButton.setBackground(AssetsColorArray[2]);
            ValidDetailsEntered = false;
        }
    }

    private final JButton ChangeEmailAddressButton = new JButton("Change email address");
    private boolean ValidDetailsEntered = false;

    private void ChangeEmailAddressAndCancelButtonSetup() {
        /*
        This method sets the properties of the change email address and cancel buttons
        It then adds them to the change email address panel
         */

        //Sets properties of the change email address button
        ChangeEmailAddressButton.setFont(AssetsFontArray[1]);
        ChangeEmailAddressButton.setBounds(520, 864, 470, 60);
        ChangeEmailAddressButton.setBackground(AssetsColorArray[0]);
        ChangeEmailAddressButton.setBorder(AssetsBorderArray[0]);
        ChangeEmailAddressButton.setFocusPainted(false);
        //Adds mouse press listener to next button
        ChangeEmailAddressButton.addActionListener((final ActionEvent ae) -> {
            ...
            if (Shape_Shooter_TD.MainClass.IsTransitionComplete()) {
                ...
                if (ValidDetailsEntered) {
                    ChangeEmailAddressButton.removeActionListener(ChangeEmailAddressButton.getActionListeners()[0]);
                    ChangeEmailAddressButton.setBackground(AssetsColorArray[1]);
                    //Locks text fields
                    ...
                    UserAccountsTableEntry ChangedUserAccountsTableEntry = new UserAccountsTableEntry(Shape_Shooter_TD.MainClass.getUsername(),
                            Shape_Shooter_TD.MainClass.Shape_Shooter_TD_Database.getCurrentUserAccountEntry().getPassword(), ValidEmailAddress);
        ...
        //Adds hover listener to change email address button that highlights it
        AddButtonHoverColorHighlight(ChangeEmailAddressButton, null);
        ChangeEmailAddressButton.setBackground(AssetsColorArray[2]);
        //Adds change email address button at front most position
        ChangeEmailAddressPanel.add(ChangeEmailAddressButton, new Integer(2));
        ...
        ChangeEmailAddressPanel.add(CancelButton, new Integer(2));
    }

    private void FinalSetup() {
        /*
        This method sets the properties of the change email address panel
        It then adds it to the main panel
         */

        //Sets change email address panel properties
        ChangeEmailAddressPanel.setBounds(0, 0, 1030, 964);
        //Adds the change email address panel to the main panel, behind the transition panel
        Shape_Shooter_TD.MainClass.MainWindowPanel.add(ChangeEmailAddressPanel, new Integer(1));
    }
}
